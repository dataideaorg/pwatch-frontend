...
export default function ExplainersPage() {
  ...
  const fetchExplainers = async () => {
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/resources/explainers/?${params}`)
      if (!response.ok) {
        throw new Error('Failed to fetch explainers')
      }

      const data: PaginatedResponse = await response.json()
      setExplainers(data.results)
      ...
      <>
        <div className="min-h-screen bg-[#f3eed4] flex items-center justify-center">
          <div className="flex flex-col items-center gap-4">
            ...
            <p className="text-[#8b7d6b]">Loading explainers...</p>
          </div>
        </div>
      </>
    )
    ...
    <div className="min-h-screen bg-[#f3eed4]">
      <main className="max-w-7xl mx-auto px-4 py-8">
        <div className="mb-8">
          ...
          <h1 className="text-3xl md:text-4xl font-bold text-gray-900">Explainers</h1>
          <p className="text-[#8b7d6b] mt-2">Educational guides and explainer documents</p>
        </div>
            ...
            <div className="flex gap-3 items-center">
              <div className="relative flex-1">
                ...
                <Input
                  ...
                  placeholder={`Search through ${totalCount} explainers...`}
                  ...
                  <tr>
                    <td colSpan={4} className="px-6 py-12 text-center">
                      <div className="flex flex-col items-center gap-3 text-[#a69780]">
                        ...
                        <p className="text-lg font-medium">No explainers found</p>
                        {searchQuery && (
                    ...
                    <tr key={explainer.id} className="hover:bg-[#f3eed4] transition-colors">
                      ...
                      <td className="px-6 py-4 text-[#8b7d6b]">
                        <div className="max-w-md">
                          {explainer.description || 'No description available'}
                        </div>
                      </td>
                      <td className="px-6 py-4 text-[#8b7d6b]">
          ...
          {explainers.length > 0 && (
            <div className="px-6 py-4 border-t border-[#d2c4b0] flex items-center justify-between">
              <div className="text-sm text-[#8b7d6b]">
                Showing {((page - 1) * pageSize) + 1} to {Math.min(page * pageSize, totalCount)} of {totalCount} explainers
              </div>
              <div className="flex items-center gap-2">
                <button
                  onClick={() => setPage(1)}
