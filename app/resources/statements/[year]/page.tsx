...
export default function StatementsYearPage() {
  ...
  const statementsForYear = useMemo(() => {
    return allStatements.filter((statement) => {
      ...
      return new Date(statement.date_received).getFullYear() === yearNumber
    })
  }, [allStatements, yearNumber])

  // Client-side filtering
  const filteredStatements = useMemo(() => {
    if (!searchQuery.trim()) return statementsForYear

    const query = searchQuery.toLowerCase()
    return statementsForYear.filter((statement) => {
      return (
        statement.name.toLowerCase().includes(query) ||
        (statement.description && statement.description.toLowerCase().includes(query))
      )
    })
  }, [statementsForYear, searchQuery])
      ...
      <div className="min-h-screen bg-[#f5f0e8]">
        <main className="max-w-7xl mx-auto px-4 py-8">
          <div className="bg-red-50 border border-red-200 rounded-lg p-4">
            ...
            <Link href="/resources/statements" className="text-[#2d5016] hover:underline mt-2 inline-block">
              Back to Statements
            </Link>
          </div>
        </main>
      ...
      <div className="min-h-screen bg-[#f5f0e8]">
        <main className="max-w-7xl mx-auto px-4 py-8">
          <div className="text-center">
            ...
            <p className="mt-4 text-gray-600">Loading statements for {year}...</p>
          </div>
        </main>
      </div>
    )
  ...
  return (
    <div className="min-h-screen bg-[#f5f0e8]">
      <main className="max-w-7xl mx-auto px-4 py-8">
        ...
        <div className="mb-8">
          <h1 className="text-3xl md:text-4xl font-bold text-gray-900 mb-2">Statements - {year}</h1>
          <p className="text-gray-600 text-lg">Official statements and position papers from CEPA for {year}</p>
        </div>
                ...
                {paginatedStatements.length === 0 ? (
                  <tr>
                    <td colSpan={4} className="px-6 py-12 text-center">
                      <div className="flex flex-col items-center gap-3 text-gray-500">
                        ...
                        <p className="text-lg font-medium">No statements found for {year}</p>
                        {searchQuery && (
                  ...
                  paginatedStatements.map((statement) => (
                    <tr key={statement.id} className="hover:bg-[#fafaf8] transition-colors">
                      ...
                      <td className="px-6 py-4 text-gray-600">
                        {statement.date_received ? formatDate(statement.date_received) : 'N/A'}
                      </td>
                      <td className="px-6 py-4">
                        {statement.file ? (
                          <a
